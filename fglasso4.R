...
#-------------------------------------------------------------------------------------------#
#           fglasso = __Main__ Algorithm                                                  #
#-------------------------------------------------------------------------------------------#
fglasso <- function(bmatrix, gamma, Theta = NULL, epsilon = 0.001, cores = 1, doprint = FALSE, 
                    iterbreak = 50, epibreak = (epsilon)*10, itertail = 5, iterlimit = 100,
                    mixture_iteration = NULL, cluster = NULL, mixture_error = NULL) {
  ...
  while (tail(norm_error, n = 1) > epsilon) {
    ...
    for (j in 1:p) {
      ...
      Theta_inv <- updateTheta(Theta = Theta, 
                               Sigma = Sigma, 
                               Theta_inv = Theta_inv, 
                               p = p, j = j, 
                               cores = cores)
      Theta <- Algorithm_3(Theta = Theta, 
                           Theta_inv = Theta_inv, 
                           SampleCov = bmatrix, 
                           gamma = gamma, 
                           epsilon = epsilon, 
      ...
      Uj <- get_U(Theta_inv = Theta_inv, 
                  Theta = Theta, 
                  p = p, M = M, j = j, 
                  cores = cores)
      Sigma <- updateSigma(Sigma = Sigma, 
                           Theta_inv = Theta_inv, 
                           sampleCov = bmatrix, 
                           Uj = Uj, 
                           p = p, j = j, 
    ...
    if (broken == TRUE) {
      ...
      fNorm <- get_Norm(Sigma = Sigma, 
                        last_sigma = last_sigma, 
                        p = p)
      norm_error[iteration] <- fNorm
      if (is.null(mixture_iteration)) {
        plot(seq(1:iteration), norm_error, 
             xlab = "Iteration", ylab = "Error")
      } else {
        plot(seq(1:iteration), norm_error, 
             xlab = "Iteration", ylab = "Error", 
             main = paste("Cluster", cluster),
             sub = paste("Mixture: iteration", mixture_iteration, "- error", mixture_error))
      }
      if (doprint == TRUE) {
        print("--------------------------------------------------")
        print("--------------------------------------------------")
      ...
      if (iteration >= iterbreak & all(tail(norm_error, itertail) <= epibreak)) {
        PrecisionMatrix <- get_Precision(Theta = Theta, 
                                         SampleCov = bmatrix, 
                                         Theta_inv = Theta_inv, 
                                         p = p, M = M)
        ...
        return(list("edges" = edges, 
                    ...
                    "normErrors" = norm_error, 
                    "finalNormError" = fNorm))
        
      } else if (iteration >= iterlimit) {
        print("Algorithm failed to converge: error 1")
        return(list("PrecisionMatrix" = NaN, 
                    "Time" = as.numeric(Sys.time()) - start_time, 
                    "numIterations" = iteration, 
                    "normErrors" = norm_error, 
                    "errorNum" = 1))
...
Algorithm_3 <- function(Theta, Theta_inv, SampleCov, gamma, epsilon, p, M, j, cores, doprint) {
  ...
  while (tail(error, n = 1) > epsilon) {
    ...
    # }
    last_w <- Theta[-j,j]
    theta_nj_transpose <- t(matrix(lapply(Theta_inv[-j,-j], t), 
                                   nrow = nrow(Theta_inv[-j,-j]), ncol = ncol(Theta_inv[-j,-j])))
    ...
    if (cores != 1) {  ### PARALLEL ###
      ...
      for(k in 1:(p-1)) {
        ...
        if (fNorm <= gamma) {
          ...
          w_j[[k]] <- matrix(nleqslv(x = initial_guess,
                                     fn = get_w_jk, 
                                     jac = NULL, 
                                     control=list(btol=.01, allowSingular = TRUE, maxit = 100),
                                     Theta_inv = Theta_inv, 
                                     SampleCov = SampleCov,
                                     block_resid = block_resid, 
                                     gamma = gamma,
                                     M = M, j = j, k = k)$x, nrow = M, ncol = M)
        }
      }
...
#-------------------------------------------------------------------------------------------#
get_w_jk <- function(x, Theta_inv, SampleCov, block_resid, gamma, M, j, k) {
  
  w <- matrix(x, nrow = M, ncol = M)
  kron_prod <- kronecker(Theta_inv[-j,-j][[k,k]], SampleCov[[j,j]])
  wj <- ((kron_prod %*% as.vector(w)) +
           as.vector(block_resid) + 
           (gamma * (as.vector(w) / norm(w, type = "F")))
         )
  return(wj)
}
...
get_Precision <- function(Theta, SampleCov, Theta_inv, p, M) {
  ...
  for (j in 1:p) {
    ...
    Theta[[j,j]] <- ( solve(SampleCov[[j,j]]) + 
                        (do.call(cbind, w_j_transpose) %*% 
                           unblock_theta_nj %*% 
                           do.call(rbind, Theta[-j,j])
                         )
                      )
  }
  return(Theta)
